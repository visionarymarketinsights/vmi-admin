...
export default function PressReleaseList() {
    ...
    const getAllpressRelease = () => {
        axios.get(`${apiUrl}/press_release/search?page=1&per_page=200&keyword=${keyword}`)
            .then(response => {
                console.log(response);
                let repList = response.data.data.map(res => {
                    res.abr = 'XXX';
